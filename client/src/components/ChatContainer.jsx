  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll flex flex-col relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        <img src={selectedUser.profilePic || assets.avatar_icon} alt="pfp" className='w-8 aspect-[1/1] rounded-full' />
        <p className='flex-1 text-lg text-white flex items-center gap-2'>
          {selectedUser.fullName} 
          {onlineUsers.includes(selectedUser._id) && <span className='w-2 h-2 rounded-full bg-green-500'></span>}
        </p>
        <img src={assets.arrow_icon} onClick={()=>setSelectedUser(null)} className='md:hidden max-w-7' />
        <img src={assets.help_icon} className='max-md:hidden max-w-5' />
      </div>

      {/* --------- Chat area --------- */}
      <div className='flex-1 flex flex-col gap-2 h-[calc(100% - 120px)] overflow-y-scroll p-3 pb-6 mb-15'>
        {messages.map((msg, index)=>(
          <div key={index} className={`flex justify-end items-end gap-2 ${msg.senderId !== authUser._id && 'flex-row-reverse'}`} >
            ...
            <div className='text-center text-xs'>
              <img src={msg.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} className='w-7 aspect-[1/1] rounded-full' />
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        ...
        <img onClick={handleSendMessage} src={assets.send_button} alt="send-icon" className='w-8 cursor-pointer' />
      </div>
    </div>
  ) : (
    <div className='flex flex-col justify-center items-center gap-2 bg-white/10 max-md:hidden'>
