    ...
    ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pb-5'>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
          ...
          <input onChange={(e)=> setInput(e.target.value)} type="text" placeholder='Search user...' className='bg-transparent border-none outline-none font-semibold text-white text-xs placeholder-[#c8c8c8] flex-1' />
        </div>
      </div>
      ...
      <div className='flex flex-col gap-4'>
        {filteredUsers.map((user, index)=>(
          ...
          ${selectedUser?._id === user._id && 'bg-[#282142]/50 rounded'}`}>
            <img src={user?.profilePic || assets.avatar_icon} alt="pfp" className='w-[35px] aspect-[1/1] rounded-full' />
